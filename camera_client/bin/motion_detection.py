...
class motionAgent:
    ...
    def detect(self, image, threshVal=50):
        ...
        delta = cv2.absdiff(self.bg.astype("uint8"), image) ##calculate the absolute difference (delta) between the background image and the current frame
        threshold =  cv2.threshold(delta, threshVal, 255, cv2.THRESH_BINARY)[1] ##returns the threshold value which is used to identify changes in the frame, we take the threshold image

        threshold = cv2.erode(threshold, None, iterations=2) ##erode the boundaries of the image, iterate over it twice
        threshold = cv2.dilate(threshold, None, iterations=2) ##dilate the image eroded image, as erosion removes whitenoise, dilation enables us to increase object area after erosion of the boundaries

        contours = cv2.findContours(threshold.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)  #get the contours from the frame -- returns a numpy array
        ...
        return (threshold, (minX, minY, maxX, maxY)) ##return the frame and the coordinates for the rectangle around the motion
    ...
    while True:
        ...
        if total > frameCount: ##this ensures we have captured a second of the video footage as we define the fps in the argument for this function
            ...
            if motion is not None:
                ...
                (thresh, (minX, minY, maxX, maxY)) = motion ##the return from our function is the image and the coordinates of the rectangle we're going to place
                cv2.rectangle(frame, (minX, minY), (maxX, maxY), (0,0,255), 2) ## draw the rectangle on the frame, it's red and it has the thickness of 2
                filename = f"{str(timestamp.strftime('%A%d%B%Y%I:%M:%S'))}{location}.png" ## set the filename with the timestamp and location
                img = motionFrame(filename, frame, store) ###create an object of the frame where motion has been detected
                img.store(img.title, img.image, img.dir) ##use the storage function of the object
                x = threading.thread(prepImage(location, timestamp, f'{store}/{filename}')) ##use the client communication module to send data to the storage server - create a multithread to do this
